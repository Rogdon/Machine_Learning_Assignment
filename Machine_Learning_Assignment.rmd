...
##Loading the Data 
We first load the taining and testing csv files into R data frames.
...
Before continuing our analysis, we are going to remove these unnecessary variables that contain little value.
...
We further subdivide the training to allow for further testing of the model fit.
...
modFit <- train(classe ~ ., data=subTrainSet,method="rf", trControl=ctrlRF, ntree=200)
modFit
```
